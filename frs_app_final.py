...
<style>
    ...
    .banner-container {{
        max-height: 50px;          /* ← Controls maximum banner height */
    ...
    .pass-btn {{ background: linear-gradient(45deg, #00cc00, #00ff00) !important; box-shadow: 0 0 15px #00ff00 !important; }}
    .not-verified-btn {{ background: linear-gradient(45deg, #cc0000, #ff0000) !important; box-shadow: 0 0 15px #ff0000 !important; }}
    .stButton>button {{ border: none !important; border-radius: 12px !important; padding: 14px !important; font-size: 1.3em !important; font-weight: bold !important; }}
...
if not st.session_state.get('authentication_status', False):
    ...
    st.markdown('<p style="text-align:center; color:#ffffff;">Pengguna perlu Log In dan masukkan kata laluan</p>', unsafe_allow_html=True)
    ...
    if st.session_state.page == 'main':
        # Banner container with max height control
            ...
            if ctx.video_processor.match_staff_id:
                ...
                if action_key == "OUT":
                    if not has_open_clock_in(staff_id):
                        ...
                        st.markdown(f"""
                        <button class="pass-btn" style="width:100%; margin:20px 0;">
                            PASS - {title} pada {ts.strftime('%H:%M HRS')}
                        </button>
                        """, unsafe_allow_html=True)
                ...
                st.markdown("""
                <button class="not-verified-btn" style="width:100%; margin:20px 0;">
                    NOT VERIFIED
                </button>
                """, unsafe_allow_html=True)
